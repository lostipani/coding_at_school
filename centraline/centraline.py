"""
centraline.py contains classes definition to fetch and analyse data from
both sensor.community devices (aka centraline) and APPA sensors.


L.Stipani, LS Galilei, Trento, 2022
lorenzo.stipani@gmail.com
"""
...
class centraline(object):
  """
  centraline object members
  -----
  date   : range of days as 'yyyy-mm-dd'
  sensor : sensor id from sensor.community
  ...
  example calls
  -------------
  test = centraline('2022-05-20', '2022-05-27')
  ...
  def __init__(self, start, end, sensor=57925):
    """
    Args
    ----
    start  : 'yyyy-mm-dd'
  ...
  def _fetch(self):
    ...
    for dd in pd.period_range(self.date[0], self.date[1]): 
      try:
        df = pd.read_csv(self._make_url(dd, self.sensor), delimiter=';')
  ...
  def _make_url(self, date, sensor):
    """
    URL string maker
    """
    filename = str(date) + '_sds011_sensor_' + str(sensor) + '.csv'
    return 'https://archive.sensor.community/' + str(date) + '/' + filename
  ...
  def plot(self, date=['', ''], hour=[0, 23], ma=0):
    ...
    Args
    ----
      ma : rolling window extent for moving average
    """
